...
Usage examples:
    PYTHONPATH=src python scripts/play_amo_genesis.py
    PYTHONPATH=src python scripts/play_amo_genesis.py --interactive
    PYTHONPATH=src python scripts/play_amo_genesis.py --vx 0.6 --yaw-rate 0.1
"""
...
from amo_policy.genesis_config import COMMAND_LAYOUT, OBS_LAYOUT
...
def _build_index_map(amo_names: list[str], robot_names: list[str]) -> tuple[list[int], list[int]]:
    """Build bidirectional index maps:
    - amo_to_robot[i] gives robot index for AMO joint i
    - robot_to_amo[j] gives AMO index for robot joint j (or -1)
    """
    robot_norm = {_norm_joint_name(n): i for i, n in enumerate(robot_names)}
    amo_to_robot = []
    for name in amo_names:
        key = _norm_joint_name(name)
        if key not in robot_norm:
            raise ValueError(
                f"AMO joint '{name}' not found in Genesis robot joints. "
                f"Available joints: {robot_names}"
            )
        amo_to_robot.append(robot_norm[key])

    robot_to_amo = [-1] * len(robot_names)
    for amo_i, robot_i in enumerate(amo_to_robot):
        robot_to_amo[robot_i] = amo_i
    return amo_to_robot, robot_to_amo
